...
def get_macd_ma(Close, Open, High, Low,
                last_dict={"dea": 0, "ema12": 0, "ema26": 0, "ma29": 0, "sum4": 0, "sum9": 0, "sum29": 0, "sum59": 0}):
    # 计算MACD
    try:
        ...
        return {"DIF": dif, "DEA": dea, "MA30": ma30, "MACD_today": macd, "MACD_yesterday": last_dict["MACD_yesterday"],
                "MA5": ma5, "MA10": ma10, "MA20": ma20, "MA30": ma30, "MA60": ma60, "MA3": ma3, "MA7": ma7,
                "MA12": ma12,
        ...
        return {"DIF": "-", "DEA": "-", "MA30": "-", "MACD_today": "-", "MACD_yesterday": "-", "MA5": "-", "MA10": "-",
                "MA20": "-", "MA30": "-", "MA60": "-", "MA3": "-", "MA7": "-", "MA12": "-",
                "ma5_yesterday": "-",
                "ma10_yesterday": "-",
                "ma20_yesterday": "-",
...
def get_last_macd(scode, start_date=""):  # 获取最近日线
    try:
        ...
        return scode, {"dea": df["DEA"].iloc[-1],
                       ...
                       "sum4": sum4, "sum19": sum19, "sum9": sum9, "sum29": sum29, "sum59": sum59, "sum2": sum2,
                       "sum6": sum6, "sum11": sum11,
        ...
        return scode, {"dea": "-",
                       ...
                       "ema12": "-",
                       "ema26": "-",
                       "sum4": [], "sum19": [], "sum9": [], "sum29": [], "sum59": [], "sum2": [], "sum6": [],
                       "sum11": [],
                       "OCHL": []}
...
def get_basic_data_info(start_time: str = "20230801"):
    url = "https://12.push2.eastmoney.com/api/qt/clist/get?pn=1&pz=10000&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&wbp2u=|0|0|0|web&fid=f3&fs=m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23,m:0+t:81+s:2048&fields=f2,f5,f3,f17,f16,f5,f15,f6,f10,f8,f14,f12,f20,f26"
    r = Requests(url)
    result_list = []

    # 将输入的 start_time 转化为 datetime 格式，方便后续比较
    start_date = datetime.datetime.strptime(start_time, "%Y%m%d")

    for item in r.json()["data"]["diff"]:
        scode = item["f12"]

        # 使用 f26 字段作为上市日期并检查是否符合条件
        list_date = None
        list_date_str = str(item["f26"])  # f26 是上市日期，已为 YYYYMMDD 格式
        try:
            list_date = datetime.datetime.strptime(list_date_str, "%Y%m%d")
        except ValueError:
            continue  # 如果转换失败，则跳过该股票

        # print(list_date)
        if list_date >= start_date:
            continue  # 如果上市时间晚于或等于输入时间，则跳过该股票

        try:
            # 市场判断
            market = "沪市" if item["f12"][0] == "6" else "深市"
            ...
            print(f"Error processing stock {scode}: {e}")
            continue

    return result_list


def get_stock_high_low(stock_code: str, hold_date: str):
    # TODO
    pass
...
if __name__ == "__main__":
    ...
    result = get_basic_data_info("20230801")
    for stock in result:
        print(stock)
    print(len(result))
